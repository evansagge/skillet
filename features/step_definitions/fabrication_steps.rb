...
Given /^a ([^"]*) exists with an? ([^"]*) of "([^"]*)"$/ do |model_name, attribute_name, attribute_value|
  create_with_default_attributes(model_name, 1, { attribute_name => attribute_value})
end

Given /^(\d+) ([^"]*) exists$/ do |count, model_name|
  create_with_default_attributes(model_name, count.to_i)
end

Given /^the following ([^"]*) exists:$/ do |model_name, table|
